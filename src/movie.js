(function () {
	...
	Gordon.Movie = function (url, options) {
		...
		for (var prop in defaults) {
			t[prop] = prop in options ? options[prop] : defaults[prop];
		}
		if (!url) {
			throw new Error("URL of a SWF movie file must be passed as first argument");
		}
		...
		new Gordon.Parser((/^\w:\/\//.test(url) ? '' : LOCATION_DIRNAME) + url, function (obj) {
			...
			if (action) {
				eval("obj.action = function(){ " + action + "; }");
			}
			switch (obj.type) {
				case "header":
					for (var prop in obj) {
				...
				case "frame":
					...
					if (n == t.totalFrames) {
						t._changeReadyState(s.COMPLETE);
					}
					break;
				default:
                    if (obj)
