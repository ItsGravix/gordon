(function () {
	var NS_SVG = "http://www.w3.org/2000/svg",
		...
		buttonMask = 0;
	for (var state in b) {
		buttonStates[b[state]] = state.toLowerCase();
	}
	...
	Gordon.SvgRenderer = function (width, height, frmSize, quality, scale, bgcolor) {
		...
		t.bgcolor = bgcolor;
		if (scale == Gordon.scaleValues.EXACT_FIT) {
			...
			if (!node) {
				...
				switch (type) {
					...
					case "radial":
						...
						if (fill.interpolation == i.LINEAR_RGB) {
							attrs["color-interpolation"] = "linearRGB";
						}
		...
		place: function (character) {
			...
			if (replace && !character.object) {
				var id = character._id,
					node = replace.node,
					matrix = character.matrix;
				if (id != replace.character._id) {
					t._setAttributes(node, {href: "#c" + id}, NS_XLINK);
				}
				var matrix = character.matrix;
				if (matrix && matrix != replace.matrix) {
					t._setAttributes(node, {transform: matrix2string(matrix)});
				}
			} else {
				if (character.clipDepth) {
					var node = t._createElement('g');
				...
				if (1 == depth) {
					...
					for (var entry in d) {
						...
						if (c.clipDepth && depth <= c.clipDepth) {
							stage = d[entry].node;
						}
						if (entry > depth) {
							nextDepth = entry;
					...
					if (nextDepth) {
						stage.insertBefore(node, d[nextDepth].node);
					} else {
		...
		_prepare: function (character) {
			...
			switch (type) {
				...
				function setState(state) {
					if (state == b.UP) {
						style.cursor = setState._cursor || "default";
					} else {
		...
		remove: function (depth) {
			...
			if (item.character.clipDepth) {
				...
				for (var c in childNodes) {
					parentNode.insertBefore(childNodes[c], node);
				}
			}
	...
	function color2string(color) {
		if ("string" == typeof color) {
			return REGEXP_IS_COLOR.test(color) ? color : null;
		}
